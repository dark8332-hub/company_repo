...
# Schema setup runs once per process, not on every connection: it issues ~40 statements
# (CREATE TABLE IF NOT EXISTS, PRAGMA table_info, migration lookups) and a single API request
# opens dozens of connections. It runs again when the database file is gone (fresh install, or
# a test that removed it); running it twice is harmless because every statement is idempotent.
_SCHEMA_READY = False
# Scheduled checks write multi-megabyte result JSON while operators browse. WAL keeps those
# readers going during the write, and the busy timeout replaces sqlite3's 5 s default so a slow
# write waits instead of failing with "database is locked".
BUSY_TIMEOUT_SECONDS = 30.0


def _connect() -> sqlite3.Connection:
    global _SCHEMA_READY
    needs_schema = not _SCHEMA_READY or not DB_FILE.exists()
...
def list_providers() -> list[dict]:
    ...
    return [{**dict(row), "available_tools": json.loads(row["available_tools"]), "sudo_mode": "authentication_required" if row["sudo_mode"] == "passwordless" else row["sudo_mode"]} for row in rows]
...
def get_provider(provider_id: str) -> dict | None:
    ...
    if data["username"] != "root":
        data["sudo_mode"] = "password" if data["credentials"].get("sudo_password") else "authentication_required"
...
def latest_check_meta(provider_id: str) -> dict | None:
    """Identity of the most recent check without loading its (multi-megabyte) result JSON."""
    with _connect() as connection:
        row = connection.execute("SELECT id,status,checked_at FROM check_results WHERE provider_id=? ORDER BY checked_at DESC LIMIT 1", (provider_id,)).fetchone()
