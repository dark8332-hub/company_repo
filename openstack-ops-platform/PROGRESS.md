...
- 시작일: 2026-08-21
- 현재 단계: 폐쇄망 반입 번들 1.1.5 생성·설치 검증 완료(런타임 번들 2.3.5 동봉). 남은 것은 실제 사이트의 sudo 비밀번호 인증 검증과 재부팅 확인, 그다음이 알림 발송 채널·다중 사용자 역할
...
15. 배포 번들을 실제 폐쇄망 사이트에서 설치 검증. 1.1.5 번들은 이 서버에서 nerdctl 로 전 과정(설치·백업·복원·재기동·유닛·제거)을 돌렸고, docker·podman 경로는 스크립트 분기까지만 맞췄다
