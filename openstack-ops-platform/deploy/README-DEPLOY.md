...
### 1.1.4 에서 1.1.5 로 올릴 때

데이터 형식은 바뀌지 않아 `data/`를 그대로 옮기면 됩니다. 다만 세 가지가 달라집니다.

**1. sudo 비밀번호가 필수입니다.** 이전 버전은 대상 노드가 NOPASSWD 면 비밀번호 없이 권한을
상승했습니다. 1.1.5 는 그 경로를 없앴고, `root` 가 아닌 계정은 항상 저장된 sudo 비밀번호를 씁니다.
NOPASSWD 로 등록해 둔 공급자는 목록에서 **sudo 비밀번호 등록** 배지로 표시되며, 점검은 권한 상승
실패로 처리됩니다. **[공급자 연결] → 해당 공급자 → sudo 비밀번호** 에서 한 번 등록하면 됩니다.
등록할 때 실제로 노드에 접속해 비밀번호를 검증하므로, 틀린 값이 저장되지 않습니다.

**2. `data/` 에 파일이 늘어납니다.** SQLite 를 WAL 모드로 바꿔 `providers.db-wal` 과
`providers.db-shm` 이 함께 생깁니다. 점검 결과를 쓰는 동안 화면 조회가 막히지 않게 하기 위한
것입니다. `./opsctl.sh backup` 은 컨테이너를 멈춘 뒤 묶으므로 이 파일들까지 일관되게 백업되고,
복원도 그대로 처리합니다. **직접 `data/` 를 복사할 때는 서비스를 멈춘 뒤 디렉터리 전체를**
복사하세요(위 절차의 `./opsctl.sh stop` 이 그 역할입니다).

**3. 브라우저 강력 새로고침이 필요 없습니다.** 화면 파일의 캐시 무효화 값을 서버가 릴리스마다
자동으로 바꿉니다. 이전에는 이 값을 손으로 올려야 했고, 빠뜨리면 브라우저가 옛 화면 코드를 계속
쓰는 일이 있었습니다.
...
| 구분 | 내용 |
|---|---|
| 권한 | sudo 비밀번호 인증으로 통일. NOPASSWD 자동 선택 제거. 비밀번호는 sudo 표준 입력으로만 전달하고 점검 스크립트의 입력은 `/dev/null` 로 고정 |
| 백업 | 임시 파일에 만든 뒤 성공했을 때만 최종 파일로 교체. 실패·중단 시 원래 실행 상태로 되돌림 |
| 복원 | 서비스를 멈추기 전에 아카이브 전체를 검증. `data/` 밖 경로·링크·특수 파일 거부. 이전 데이터는 `data.before-restore.*` 로 보존 |
| 안전장치 | 모든 운영 명령이 이미지 저장소와 관리 라벨로 컨테이너 소유를 확인. 포트가 물려 있으면 기존 컨테이너를 지우지 않고 중단 |
| 추적 | 이미지에 릴리스 버전·소스 커밋·빌드 시각을 기록. `./opsctl.sh status` 와 `/api/health` 에서 확인 |
| 성능 | SQLite WAL 적용, 점검 결과 저장·조회를 별도 스레드로 분리 |

`VERSION` 파일에 이 번들이 어느 커밋에서 나왔는지 적혀 있습니다.
